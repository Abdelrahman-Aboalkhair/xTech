...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ src }) => {
  ...
  const waveformBars = Array.from(
    { length: 40 },
    () => Math.floor(Math.random() * 100) + 10 // Random height between 10 and 110
  );
  ...
  const formatTime = (time: number) => {
    const minutes = Math.floor(time / 60); // Convert to minutes
    const seconds = Math.floor(time % 60); // Get the remaining seconds
  ...
  // Fix for useEventListener hook with audio element
  useEffect(() => {
